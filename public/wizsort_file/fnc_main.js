...
function startup() {
   ...
   new_CheckBox.setAttribute('type', 'checkbox', 0);
   new_CheckBox.setAttribute('checked', 'true', 0);
   new_CheckBox.value = "全て";
   new_CheckBox.title = "チェックすることで、全てのチェックボックスを同時に変更します。";
   new_CheckBox.id = 'optSelect_all';
   new_CheckBox.onclick = function() {chgAll();}
   new_cell.appendChild(new_CheckBox);

   var new_span = cE('span');
   new_span.appendChild(cT("すべて"));
   new_cell.appendChild(new_span);
...
function imginit() {
   ...
   for (i=0; i<ary_CharacterData.length; i++) {
      ...
      if (ary_CharacterData[i][5].length > 0) {
         for (j=5; j<ary_CharacterData[i].length;j++) {
            var new_img = cE('img');
...
function fnc_ShowData() {
   ...
   if (int_Status == 2) {
      ...
      new_cell.appendChild(cT('順位'));
      // Col[1]
      new_cell = new_row.insertCell(new_row.childNodes.length);
      sC(new_cell, 'resTableH');
      new_cell.appendChild(cT('名前'));
      ...
      for (i=0; i<ary_TempData.length; i++) {
         ...
         if ((int_ResultImg != 0) && (i < int_ResultRank)) {
            ...
            if (obj_TempData[5].length > 0) {
               new_img.src = str_ImgPath + obj_TempData[Math.floor(Math.random() * (obj_TempData.length -5)) + 5];
               new_cell.appendChild(new_img);
               new_cell.appendChild(cE('br'));
               bln_imgFlag = true;
            }
         ...
         if ((int_ResultImg == 2) || (!bln_imgFlag)) {
			...
			csort4[i] = ary_TempData[ary_SortData[0][i]][1];
			csort6[i] = ary_TempData[ary_SortData[0][i]][1];			
         }
      ...
      for (i=0; i<2; i++)
	   {
         ...
         if ((obj_TempData[5].length > 0) && gID('optImage').checked)
		  {
            var obj_Item = cE("img");
			csort7 = Math.floor(Math.random() * (obj_TempData.length - 5)) + 5
            obj_Item.src = str_ImgPath + obj_TempData[csort7];		
            obj_Item.title = obj_TempData[1];
			obj_Item.alt = obj_TempData[1];
         ...
         var obj_SelectItem = gID((i == 0) ? "linLeft" : "linRight");
         var obj_TempData = ary_TempData[ary_SortData[(i == 0)  ? int_LeftList : int_RightList][(i == 0)  ? int_LeftID : int_RightID]];
         if ((obj_TempData[5].length > 0) && gID('optImage').checked)
		  {
			 
			csort8 = csort7-5;		       
			var grah_date = obj_TempData[4][csort8];
			if( grah_date != "#")
			{
				var obj_Item = cE("div");
				var myinput = document.createElement("a");
				myinput.href = grah_date;
				myinput.target = "_blank" ;
				myinput.innerHTML = obj_TempData[3][csort8]; 
				obj_Item.appendChild(myinput);
				obj_Item.className = "graphic" ;
			}
			else
			{
				var obj_Item = cE("div");
				//obj_Item.href = grah_date;
				//obj_Item.onclick = MyAlert4;
				var myinput = document.createElement("span");
				myinput.innerHTML = obj_TempData[3][csort8]; 	
				obj_Item.appendChild(myinput);
				obj_Item.className = "graphic2" ;
